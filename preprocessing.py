...
def create_vector_db():
    loader = DirectoryLoader(DATA_PATH,
                             glob='*.pdf',
                             loader_cls=PyPDFLoader) # Load the PDFs present in the directory

    documents = loader.load()
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=500,
                                                   chunk_overlap=50) # Initialize the text_splitter function
    texts = text_splitter.split_documents(documents)# Split the loaded documents into chunks 

    embeddings = HuggingFaceEmbeddings(model_name='sentence-transformers/all-MiniLM-L6-v2',
                                       model_kwargs={'device': 'cpu'}) # Initialize our sentence transformers model along with the device to be used 
